...
export function useSwapCallback(
  ...
  const eip1559 =
    EIP_1559_ACTIVATION_BLOCK[chainId] === undefined ? false : currentBlock >= EIP_1559_ACTIVATION_BLOCK[chainId]

  const useArcher = archerRelayDeadline !== undefined

  const swapCalls:any = useSwapCallArguments(trade, allowedSlippage, recipientAddressOrName, signatureData, useArcher)
    ...
    return {
      ...
      callback: async function onSwap(): Promise<string> {
        const estimatedCalls: SwapCallEstimate[] = await Promise.all(
          swapCalls.map((call: { address: any; calldata: any; value: any }) => {
